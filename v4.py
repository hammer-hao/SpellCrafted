...
#import BERT pretrained tokenizer
...
padded_text_list=[torch.cat((item, torch.full((max_len - len(item),), 0))) for item in encoded_text_list] # the [PAD] token has id=3
...
def generate(cardname):
    context= torch.tensor([encode(f'{cardname}: [SEP]')], dtype=torch.long, device=device)
